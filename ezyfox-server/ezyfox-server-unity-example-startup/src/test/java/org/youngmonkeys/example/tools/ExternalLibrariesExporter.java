package org.youngmonkeys.example.tools;
        ...
        if (exportedLibraries.isEmpty()) {
            System.err.println(
                "you must run: "
                    ...
                    + "on ezyfox-server-unity-example-startup module to export all libraries first"
            );
            return;
        }
    ...
    private static Path getDeployLibFolderPath() {
        ...
        if (!Files.exists(deployLibFolderPath)) {
            deployLibFolderPath = Paths.get(
                "ezyfox-server-unity-example-startup/deploy/lib"
            );
        }
